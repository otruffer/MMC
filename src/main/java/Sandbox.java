...
public class Sandbox {
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		 * Person profile = plus.people().get("me").execute(); show(profile);
		 * 
		 * ActivityFeed feed = plus.activities().list(profile.getId(),
		 * "public").execute(); for (Activity activity : feed.getItems()) {
		 * show(activity); String id = activity.getId(); PeopleFeed people =
		 * plus.people().listByActivity(id, "plusoners").execute();
		 * System.out.println("PEOPLE FOR THIS ACTIVITY"); for(Person p :
		 * people.getItems()){ show(p); } }
		 */
	}
